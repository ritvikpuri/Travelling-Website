...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/registration").permitAll().and().formLogin()
				.loginPage("/login").and().csrf().disable();
		http.authorizeRequests().antMatchers("/book","/selected","/selected/{flightNum}","/payment").authenticated();
	}
